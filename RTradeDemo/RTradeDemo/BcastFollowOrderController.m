...
#import "CellBcastFollowOrderCell.h"
#import "TradeUtility.h"

@interface BcastFollowOrderController ()

@end

@implementation BcastFollowOrderController
     NSArray *followOrderData;
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)showFollowOrderList
{
    ...
    if(icode == 0)
    {
        ...
        if(retjson != nil)
        {
            followOrderData = [retjson objectForKey:@"live_list"];
...
#warning Incomplete implementation, return the number of rows
    if(followOrderData != nil)
    {
        return followOrderData.count;
    }
    else
    {
        return 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CellBcastFollowOrderCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CellBcastFollowOrderCell" forIndexPath:indexPath];
    
    // Configure the cell...
    if(followOrderData != nil)
    {
        NSDictionary *itemData = [followOrderData objectAtIndex:indexPath.row];
        ...
        cell.m_strContracts.text = [ NSString stringWithFormat : @"%@",[itemData objectForKey:@"contracts"]];
