...
- (instancetype)initWithItems:(NSArray <NSString *>*)titleItems distributionStyle: (YYTopBarDistributionStyle)distributionStyle {
    ...
    if (self) {
        ...
        _colorArray = [NSMutableArray arrayWithObjects:[UIColor redColor],[UIColor greenColor],[UIColor YYStock_MA5LineColor], nil];
        _wordsArray = [NSMutableArray arrayWithObjects:@"多",@"空",@"平", nil];
        [self setupHeadView];
    }
    return self;
}
...
//        [btn setTitle:title forState:UIControlStateNormal];
        ...
        for (NSInteger i=0; i<3; i++) {
            UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(i*40, 16, 8, 8)];
            label.layer.cornerRadius = 4;
            label.layer.masksToBounds = YES;
            label.backgroundColor = _colorArray[i];
            [btn addSubview:label];
            UILabel *label1 = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMaxX(label.frame)+2, 12, 15, 15)];
            label1.font =[UIFont systemFontOfSize:12];
            label1.textColor = [self textNormalColor];
            label1.text = _wordsArray[i];
