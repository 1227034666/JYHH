...
//    CGContextRef ctx = UIGraphicsGetCurrentContext();
    ...
    __block NSString *lastDateLabel;
...
//        }
        ...
        if (pModel.DayDesc.length > 0) {
            NSDictionary *attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:10],NSForegroundColorAttributeName:[UIColor redColor]};
            
            NSString *dayLabel=pModel.DayDesc;
//            NSLog(@"dayLabel: %@",dayLabel);
            if ([YYStockVariable lineWidth] < 1) {
                dayLabel = [NSString stringWithFormat:@"%@/%@",[pModel.DayDesc substringWithRange:NSMakeRange(4, 2)],[pModel.DayDesc substringWithRange:NSMakeRange(6, 2)]];
                CGFloat _lineWth = [YYStockVariable lineWidth];
//                NSLog(@"绘制日期 %.2f", _lineWth);
            } else{
               dayLabel = [NSString stringWithFormat:@"%@:%@",[pModel.DayDesc substringWithRange:NSMakeRange(8, 2)],[pModel.DayDesc substringWithRange:NSMakeRange(10, 2)]];
            }
            if ([lastDateLabel isEqualToString:dayLabel]) {
                return;
            } else{
            ...
            CGRect rect = [dayLabel boundingRectWithSize:CGSizeMake(MAXFLOAT, 0)
            ...
            if (idx == 0) {
                [dayLabel drawAtPoint:CGPointMake(pModel.StartPoint.x+1, pModel.EndPoint.y+5) withAttributes:attribute];
            } else if(idx + 1 == [self.drawPositionModels count]) {
                [dayLabel drawAtPoint:CGPointMake(pModel.StartPoint.x - width, pModel.EndPoint.y + 5) withAttributes:attribute];
            } else {
                [dayLabel drawAtPoint:CGPointMake(pModel.StartPoint.x - width/2, pModel.EndPoint.y + 5) withAttributes:attribute];
            }
        }
    }];
