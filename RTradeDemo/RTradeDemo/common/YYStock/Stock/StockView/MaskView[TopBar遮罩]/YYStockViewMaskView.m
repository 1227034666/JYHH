...
- (void)drawTimeLineMask:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    CGRect drawRect= CGRectMake(5, (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
    ...
    [self.drawTimeLineDescTexts enumerateObjectsUsingBlock:^(NSString *  _Nonnull drawText, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect textRect = [self rectOfNSString:drawText attribute:attribute];
        CGRect drawRect = CGRectMake(45 + gap * idx , (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
        [drawText drawInRect:drawRect withAttributes:attribute];
    }];
    ...
    [[self drawTimeLineTexts] enumerateObjectsUsingBlock:^(NSString *  _Nonnull drawText, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect textRect = [self rectOfNSString:drawText attribute:attribute];
        CGRect drawRect = CGRectMake(45 + 45 + gap * idx , (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
        [drawText drawInRect:drawRect withAttributes:attribute];
    }];
...
- (void)drawLineMask:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    [self.drawKlineDescTexts enumerateObjectsUsingBlock:^(NSString *  _Nonnull drawText, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect textRect = [self rectOfNSString:drawText attribute:attribute];
        CGRect drawRect = CGRectMake(110 + gap * idx , (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
        [drawText drawInRect:drawRect withAttributes:attribute];
    }];
    ...
    [[self drawKlineTexts] enumerateObjectsUsingBlock:^(NSString *  _Nonnull drawText, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect textRect = [self rectOfNSString:drawText attribute:attribute];
        CGRect drawRect = CGRectMake(110 + 50 + gap * idx , (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
        [drawText drawInRect:drawRect withAttributes:attribute];
    }];
...
- (NSArray *)drawTimeLineDescTexts {
    if (!_drawTimeLineDescTexts) {
        _drawTimeLineDescTexts = @[@"成交价:",@"涨跌率:",@"昨结价:"];
    }
...
- (NSArray *)drawTimeLineTexts {
    return @[
             [NSString stringWithFormat:@"%.1f",[((id<YYStockTimeLineProtocol>)self.selectedModel).Price floatValue]],
             ...
             [NSString stringWithFormat:@"%.1f",((id<YYStockTimeLineProtocol>)self.selectedModel).AvgPrice],
             ];
}
@end
