...
//        make.height.equalTo(_stockScrollView.contentView).offset(-15);
        make.height.equalTo(_stockScrollView.contentView).multipliedBy(0.9);
    }];
    ...
    [_volumeView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(_stockScrollView.contentView).multipliedBy(0.1);
...
- (void)drawLeftDesc {
    ...
    CGFloat unit = (self.stockScrollView.frame.size.height)* 0.9f / 6.f;
    CGFloat unitValue = (maxValue - minValue)/6.f;
    //顶部间距
    for (int i = 0; i < 7; i++) {
        NSString *text = [NSString stringWithFormat:@"%.2f",maxValue - unitValue * i];
//        CGPoint drawPoint = CGPointMake((YYStockScrollViewLeftGap - textSize.width)/2, unit * i + 10 - textSize.height/2.f);
        CGPoint drawPoint = CGPointMake((YYStockScrollViewLeftGap - textSize.width)/2, unit * i + 10 - textSize.height+1);
...
- (void)event_longPressAction:(UILongPressGestureRecognizer *)longPress {
    ...
    if(UIGestureRecognizerStateChanged == longPress.state || UIGestureRecognizerStateBegan == longPress.state) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"TopBarSelected" object:@{@"selectedIndex":@"2"}];
    ...
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"TopBarSelected" object:@{@"selectedIndex":@"3"}];
        //恢复scrollView的滑动
        selectedModel = self.drawLineModels.lastObject;
        [self setNeedsDisplay];
...
-(void)setPointsArray:(NSMutableArray *)pointsArray{
    self.kLineView.pointsArray = pointsArray;
    [self setNeedsDisplay];
}
