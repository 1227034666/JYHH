...
//            NSDictionary *attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:10],NSForegroundColorAttributeName:[UIColor YYStock_topBarNormalTextColor]};
            ...
            if (pModel.StartPoint.x - width/2.f < 0) {
                ...
                //最右边判断
                [pModel.DayDesc drawAtPoint:CGPointMake(pModel.StartPoint.x - width-5, pModel.EndPoint.y) withAttributes:attribute];
            } else {
                if ([pModel.DayDesc isEqualToString:@"23:00"]) {
                   [pModel.DayDesc drawAtPoint:CGPointMake(pModel.StartPoint.x - width/2.f -8, pModel.EndPoint.y) withAttributes:attribute];
                } else{
...
//    CGFloat maxY = self.frame.size.height/2.0f;
...
//        CGFloat yPosition = 2.0f;
        
//        CGPoint startPoint = CGPointMake(xPosition, ABS(yPosition - maxY) > 1 ? yPosition : maxY );
        CGPoint startPoint = CGPointMake(xPosition, 0);
        CGPoint endPoint = CGPointMake(xPosition, 2);
        
        NSString *dayDesc = model.isShowTimeDesc ? model.TimeDesc : @"";
//        NSLog(@"S x %.f, Y: %.f  || E x %.f, Y: %.f || dayDesc %@ ",startPoint.x,startPoint.y,endPoint.x,endPoint.y, dayDesc);
